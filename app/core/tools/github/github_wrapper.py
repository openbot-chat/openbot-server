...
class GitHubAPIWrapper(BaseModel):
    ...
    def get_issues(self) -> str:
        page = self.github_repo_instance.get_issues(state="open")
        """
    ...
    def get_commits(self, author: Optional[str] = None, since: Optional[datetime] = None, until: Optional[datetime] = None):
        _author = author if author is not None else GithubObject.NotSet
        _since = since if since is not None else GithubObject.NotSet
        _until = until if until is not None else GithubObject.NotSet

        page = self.github_repo_instance.get_commits(author=_author, since=_since, until=_until)
