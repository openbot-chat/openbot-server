...
class AsyncOpenAIAssistantRunnable(RunnableSerializable[Dict, OutputType]):
    ...
    Example using OpenAI tools:
        .. code-block:: python
            ...
            interpreter_assistant = await OpenAIAssistantRunnable.acreate_assistant(
    ...
    Example using custom tools and custom execution:
        .. code-block:: python
            ...
            async def execute_agent(agent, tools, input):
    ...
    client: openai.AsyncOpenAI = Field(default_factory=_get_openai_client)
    """OpenAI client."""
    assistant_id: str
    """OpenAI assistant id."""
    ...
    def invoke(self, input: dict, config: Optional[RunnableConfig] = None) -> OutputType:
        ...
        return runnify(self.ainvoke)(input, config)
    ...
    async def ainvoke(
        ...
        # await self.client.beta.threads.runs.cancel(run_id='run_OkVhjhDe3LSvTCLvGSf8c2cl', thread_id=input['thread_id'])
        ...
        try:
            ...
            if self.as_agent and input.get("intermediate_steps"):
                tool_outputs = await self._parse_intermediate_steps(
                    input["intermediate_steps"]
                )
                run = await self.client.beta.threads.runs.submit_tool_outputs(**tool_outputs)
            ...
            # Starting a new run in an existing thread.
                ...
                _ = await self.client.beta.threads.messages.create(
                    input["thread_id"],
                    content=input["content"],
                    role="user",
                    file_ids=input.get("file_ids", []),
            ...
            # framework.
                ...
                run = await self.client.beta.threads.runs.submit_tool_outputs(**input)
            run = await self._wait_for_run(run.id, run.thread_id)
        except BaseException as e:
            await run_manager.on_chain_error(e)
            raise e
        try:
            response = await self._get_response(run)
        except BaseException as e:
            await run_manager.on_chain_error(e, metadata=run.dict())
            raise e
        else:
            await run_manager.on_chain_end(response)
            return response

    async def _parse_intermediate_steps(
        ...
        tool_outputs = [
            {"output": output if isinstance(output, str) else json.dumps(output, default=lambda obj: obj.__dict__, sort_keys=True, indent=4), "tool_call_id": action.tool_call_id}
    ...
    async def _create_run(self, input: dict) -> Any:
        params = {
            k: v
    ...
    async def _create_thread_and_run(self, input: dict, thread: dict) -> Any:
        params = {
            k: v
            for k, v in input.items()
            if k in ("instructions", "model", "tools", "run_metadata")
    ...
    async def _get_response(self, run: Any) -> Any:
        # TODO: Pagination
        ...
        if run.status == "completed":
            ...
            new_messages = [msg for msg in response.data if msg.run_id == run.id]
            ...
            for tool_call in run.required_action.submit_tool_outputs.tool_calls:
                ...
                print('requires_action tool_call: ', tool_call)
    ...
    async def _wait_for_run(self, run_id: str, thread_id: str) -> Any:
        in_progress = True
        while in_progress:
            run = await self.client.beta.threads.runs.retrieve(run_id, thread_id=thread_id)
